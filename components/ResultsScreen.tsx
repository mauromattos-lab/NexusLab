...
const KpiCard: React.FC<{ value: string; label: string; icon: React.ReactNode, color: string }> = ({ value, label, icon, color }) => (
    <div className="glass-pane rounded-xl p-4 flex flex-col justify-between h-full">
        <div>
            <div className={`w-8 h-8 flex items-center justify-center rounded-lg mb-4 text-white ${color}`}>{icon}</div>
            <p className="text-xs text-slate-400">{label}</p>
        </div>
        <p className={`text-xl md:text-2xl font-bold ${color === 'text-fuchsia-400' ? 'text-glow-secondary' : 'text-glow'}`}>{value}</p>
    </div>
);
...
const SolutionCard: React.FC<{ solution: AISolution, icon: React.ReactNode }> = ({ solution, icon }) => (
  <div className="glass-pane rounded-2xl p-6 transition-all duration-300 hover:border-cyan-400/50 hover:shadow-cyan-500/10">
    <div className="flex flex-col md:flex-row gap-6">
        <div className="w-12 h-12 flex-shrink-0 flex items-center justify-center rounded-xl bg-slate-800 text-cyan-400">{icon}</div>
        <div className="flex-grow">
            <div className="flex justify-between items-start mb-3 gap-2">
                <h3 className="text-lg md:text-xl font-bold text-white pr-4">{solution.title}</h3>
                <div className="flex-shrink-0">
                    <ImpactBadge impact={solution.impact} />
                </div>
            </div>
            <p className="text-slate-400 mb-4 text-sm">{solution.description}</p>
    ...
    <>
      ...
      <div className="w-full h-full flex flex-col p-2 animate-fade-in-up min-h-0">
        <header className="text-center mb-6 flex-shrink-0">
          ...
          <p className="text-slate-300">Análise estratégica de implementação de IA para seu negócio</p>
        </header>
        
        <main className="flex-1 overflow-y-auto pr-2">
          {/* KPI Section */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
              <div className="lg:col-span-1 glass-pane rounded-2xl p-6 flex flex-col items-center justify-center">
                  <Gauge score={result.potentialTransformationScore} />
                  <p className="mt-4 font-semibold">Potencial de Transformação IA</p>
              </div>
          ...
          <div className="mb-8 glass-pane rounded-2xl p-6 text-center">
              <h3 className="text-xl sm:text-2xl font-bold text-white mb-4">Transforme Análise em Resultados</h3>
              <p className="text-slate-400 max-w-2xl mx-auto mb-6">O diagnóstico é o primeiro passo. A consultoria estratégica é a rota mais rápida para transformar estes insights em um plano de implementação personalizado.</p>
              <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
                  <button onClick={handleDownload} disabled={isDownloading} className="w-full sm:w-auto px-6 py-3 border-2 border-cyan-400 text-cyan-400 font-semibold rounded-lg hover:bg-cyan-400 hover:text-slate-900 transition-colors duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-wait">
                      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                      {isDownloading ? 'Gerando Relatório...' : 'Baixar Diagnóstico Detalhado'}
                  </button>
                  <a href={APP_CONFIG.calendlyLink} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto text-center px-6 py-3 bg-gradient-to-r from-cyan-400 to-fuchsia-500 text-white font-semibold rounded-lg shadow-lg hover:scale-105 transition-transform duration-200 flex items-center justify-center gap-2">
         ...
         <div className="flex-shrink-0 mt-6 text-center">
             <button onClick={() => { onReset(); audioManager.playSound('transition'); }} className="text-slate-500 hover:text-cyan-300 transition-colors group text-sm">
